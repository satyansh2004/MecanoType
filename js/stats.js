...
export const CHAR_FILTERS = {
  ...
  symbols: (char) =>
    !/[a-zñA-ZÑ0-9áéíóúüàèìòùâêîôûäëïöüÁÉÍÓÚÜÀÈÌÒÙÂÊÎÔÛÄËÏÖÜ]/.test(char),
};
...
export function renderUserStats() {
  ...
  document.getElementById("profile-completed").textContent =
    userStats.completed;
  document.getElementById("profile-time").textContent = formatTime(
    userStats.time,
  );
...
/**
 * Reset user statistics to localStorage
 */
...
export function checkAndSaveRecord(wordCount, wpm, accuracy) {
  ...
  if (
    !userStats.records[count] ||
    typeof userStats.records[count] !== "object" ||
    !("wpm" in userStats.records[count])
  ) {
